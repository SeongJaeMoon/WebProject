...
class WC_Gateway_Iamport_Subscription_Ex extends Base_Gateway_Iamport {
	...
	public function iamport_payment_info( $order_id ) {
		...
		if ( in_array($this->pg_provider, array('danal_tpay')) || ($this->isMobile() && $this->pg_provider === 'html5_inicis') ) {
			$iamport_info['amount'] = 0; //JTNet, 다날-휴대폰, 이니시스-PC결제의 경우 원래 가격 그대로 설정
		}
		...
		if ( !empty($http_param['imp_uid']) ) {
			...
			if ( $result->success ) {
				...
				if ( $payment_data->status === 'paid' ) {
					...
					if ( in_array($payment_data->pg_provider, array('jtnet', 'danal')) ) { //JTNet, 다날-휴대폰 소액결제 정기결제는 이미 결제를 하고 돌아옴
			...
			$called_from_iamport ? exit('IamportForWoocommerce 2.0.57') : wp_redirect( $default_redirect_url );
		} else {
			//just test(아임포트가 지원하는대로 호출되지 않음)
			exit( 'IamportForWoocommerce 2.0.57' );
		}
	}
